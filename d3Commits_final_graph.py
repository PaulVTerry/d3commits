...
def getMode(inputList):
	...
	if maxCount == 1:
		...
		plt.title('2017 Number of Commits by Day of Week')
...
for commit in raw:
	...
	if commitDate.year == 2017:
		...
		daysWithCommits.append(commitDate.isocalendar()[2])
...
while r.links['next']['url'] != r.links['last']['url']:
	...
	for commit in raw:
		...
		if commitDate.year == 2017:
			...
			lastYearsList.append(commit)
			# adds week number to daysWithCommits list again
			daysWithCommits.append(commitDate.isocalendar()[1])
